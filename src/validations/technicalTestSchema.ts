...
const languages = ["Java", "PHP", "JavaScript", "TypeScript", "Python", "SQL"] as const;
...
export const technicalTestSchema = z.object({
  ...
  content: z
    ...
    .max(2000, { message: "El contenido debe tener menos de 2000 caracteres" })
    .optional(),
  ...
  file: z
    ...
    .refine((files) => !files || files.length === 0 || 
      ["application/pdf", "text/plain", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"].includes(files[0].type), {
      message: "Solo se permiten archivos PDF, TXT, DOC o DOCX",
    }),
}).refine((data) => {
  if (data.contentType === "text" && (!data.content || data.content.trim() === "")) {
    return false;
  }
